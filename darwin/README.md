...
machine-independent; everything machine-specific (hostname, username,
platform, git identity) is set per-block via `specialArgs` and inline
home-manager config in `flake.nix`.
...
- **`nix-dokken-dev.url`** is temporarily pointed at a local checkout
  ...
  work before that repo is pushed. Switch it back once it is. Note this also
  means evaluating **any** output of `darwin/flake.nix` — including
  `arne-mac`'s — requires that local path to exist, since flake inputs are
  resolved as a whole regardless of which output you build; see the note in
  [../README.md](../README.md) about why the NixOS `gamix` config lives in a
  separate top-level flake instead of one shared with this input.
